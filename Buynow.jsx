...
function datacmp(el, temCart, data) {
  data.map((item) => (item.name === el ? temCart.push(item) : null));
}
...
export default function Buynow() {
  ...
  return (
    <div className="footerfix ">
      ...
      <div className=" mt-50 total green">
        <h2>Total order value ₹ {totalprice}</h2>
      </div>
              ...
              <tbody>
                {cartItems.map((item, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <li key={i}>
                        <img id="iconimage" src={item.img} alt="not found" />
                        <hr />
                      </li>
                    </td>

                    <td>
                      <div className="item_hi" key={i}>
                        {item.name}
                        <hr />
                      </div>
                    </td>
                    <td>
                      <div className="green bold item_hi" key={i}>
                        {item.price+"rs"}
                        <hr />
                      </div>
                    </td>
                    <td>
                      <div className="mt-50" key={i}>
                        <button onClick={() => removeItem(i)} id="remove">
                          x
                        </button>
                      </div>
                    </td>
      ...
      <p className="red mt-50 ml-5">
        To order anonymously, you can place your order without login. Log in
        only if you want to track your order.
        <Link to="/login">Click on this</Link> for login and use the same email
        in Checkout input
      </p>
    </div>
  );
